...
import { loadBalances, transferTokens, loadBalancesNew} from '../store/interactions'
import gc from '../assets/dapp.svg'
...
const Balance = () => {
  ...
  useEffect(() => {
    if(exchange && tokens[0] && tokens[1] && tokens[2] && account) {
      loadBalances(dispatch, exchange, tokens, account)
      loadBalancesNew(dispatch, exchange, tokens, account)
    }
    ...
    <div className='component exchange__transfers'>
      ...
      <div className='exchange__transfers--form'>
        <div className='flex-between'>
          <p><small>Token</small> <br/> <img src={gc} alt='Token Logo' />{symbols && symbols[0]}</p>
          <p><small>Wallet</small> <br/> {tokenBalances && tokenBalances[0]}</p>
          <p><small>Exchange</small> <br/> {exchangeBalances && exchangeBalances[0]}</p>
        </div>

        <form onSubmit={(e) => transferHandler(e, tokens[0], isDeposit)}>
      ...
      <div className='exchange__transfers--form'>
        ...
        <form onSubmit={(e) => transferHandler(e, tokens[1], isDeposit)}>
          <label htmlFor="token1"></label>
          <input type="text" id='token1' placeholder='0.0000' value={token1TransferAmount === 0 ? '' : token1TransferAmount} onChange={(e) => amountHandler(e, tokens[1])}/>
